...
class SaleOrderLineBinauralVentas(models.Model):
    ...
    @api.depends('order_id.foreign_currency_rate','discount','price_unit', 'product_uom_qty','foreign_price_unit')
    def _amount_all_foreign(self):
        ...
        for order in self:
            ...
            if name_foreign_currency:
                ...
                discount_foreign = ((order.discount * 1/100) * order.foreign_price_unit) * order.product_uom_qty
                order.foreign_subtotal = (order.foreign_price_unit * order.product_uom_qty) - discount_foreign
    ...
    def _onchange_product_id_check_availability(self):
        not_qty_on_hand_less_than_zero_sale = self.env["ir.config_parameter"].sudo().get_param(
            "not_qty_on_hand_less_than_zero_sale"
        )
        ...
        if self.product_id.type == 'product':
            ...
            if (
                    float_compare(product.free_qty, product_qty, precision_digits=precision) == -1
                    and not_qty_on_hand_less_than_zero_sale
                ):
    ...
    foreign_price_unit = fields.Monetary(
        string='Precio Alterno', store=True,  digits=(12,2),readonly=True, compute='_amount_all_foreign', tracking=4,currency_field='foreign_currency_id')
    foreign_subtotal = fields.Monetary(
        string='Subtotal Alterno', store=True, readonly=True, digits=(12,2), compute='_amount_all_foreign', tracking=4,currency_field='foreign_currency_id')
