...
class SaleOrderBinauralVentas(models.Model):
        ...
        for order in self:
            ...
            for line in order.order_line:
                ...
                taxes = line.tax_id.compute_all(
                    line.foreign_price_unit,
                    line.order_id.foreign_currency_id,
                    line.product_uom_qty,
                    product=line.product_id,
                    partner=line.order_id.partner_shipping_id,
                )
                foreign_amount_tax += sum(t.get('amount', 0.0) for t in taxes.get('taxes', []))
    ...
    foreign_amount_total = fields.Monetary(
        string='Total moneda alterna', store=True, readonly=True, compute='_amount_all_foreign', tracking=4,currency_field='foreign_currency_id')
    ...
    @api.depends('order_line.foreign_subtotal','order_line.price_subtotal', 'order_line.price_tax', 'order_line.tax_id', 'partner_id', 'currency_id')
        ...
        for move in self:
            ...
            for line in tax_lines:
                res.setdefault(line.tax_id.tax_group_id, {
                               'base': 0.0, 'amount': 0.0,'foreign_amount':0.0,'foreign_base':0.0})
                ...
                res[line.tax_id.tax_group_id]['foreign_base'] += tax_balance_multiplicator * \
                ...
                res[line.tax_id.tax_group_id]['foreign_amount'] += amount * value_rate
                ...
                for tax in line.tax_id.flatten_taxes_hierarchy():
                    if tax.tax_group_id not in res or tax.tax_group_id in zero_taxes:
                        ...
                        res[tax.tax_group_id]['base'] += tax_balance_multiplicator * \
                            (line.price_subtotal if line.currency_id else line.price_subtotal)
                        res[tax.tax_group_id]['foreign_base'] += tax_balance_multiplicator * \
                            (line.price_subtotal) * value_rate
            ...
            move.foreign_amount_by_group = [
                (
                    group.name, amounts['foreign_amount'],
                    amounts['foreign_base'],
                    formatLang(
                        lang_env, 
                        amounts['foreign_base'] * move.order_line.filtered(lambda l: l.tax_id.tax_group_id.id == group.id)[0].tax_id.amount / 100,
                    ),
                    # formatLang(
