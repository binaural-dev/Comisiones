...
class AccountRetentionBinauralLineFacturacion(models.Model):
            ...
            if not self.env.context.get("noonchange"):
                if foreign_currency_id == 2:
                    value_rate = decimal_function.getCurrencyValue(
                        rate=record.invoice_id.foreign_currency_rate,
                        base_currency="VEF",
                        foreign_currency="USD",
                    )
                    if (
                        record.retention_amount > record.iva_amount
                        and record.retention_id.type_retention in ["iva"]
                    ):
                        return {
                            "warning": {
                            ...
                            "value": {"retention_amount": 0},
                        }
                    if (
                        float_compare(
                            record.retention_amount,
                            record.invoice_id.amount_residual,
                            precision_digits=2,
                        )
                        == 1
                        and record.retention_id
                    ):
                        return {
                            "warning": {
                                "title": "El monto retenido excedende",
    ...
    def _onchange_porcentage_retention(self):
        ...
        for record in self:
            if foreign_currency_id == 3:
                return {
                    "value": {
                        "retention_amount": record.facture_amount
                        * (record.porcentage_retention / 100)
                    },
                }
            elif foreign_currency_id == 2:
                return {
                    "value": {
                        "foreign_retention_amount": record.facture_amount
                        * (record.porcentage_retention / 100)
                    },
                }
    ...
    def _get_value_related(self):
        ...
        for record in self:
            if (
                record.retention_id
                and record.retention_id.type_retention in ["islr"]
                and record.retention_id.type in ["in_invoice"]
            ) or (
                record.invoice_id
                and record.invoice_id.move_type in ["in_invoice", "in_refund"]
                and not record.retention_id
            ):
                ...
                if record.payment_concept_id:
                    for line in record.payment_concept_id.line_payment_concept_ids:
                        if (
                            record.invoice_id.partner_id.type_person_ids.id
                            == line.type_person_ids.id
                        ):
                ...
                if (
                    record.invoice_id
                    and record.facture_amount == 0
                    or record.foreign_facture_amount == 0
                ):
                ...
                if record.payment_concept_id and record.invoice_id:
                    if record.facture_amount > record.related_pay_from:
                        ...
                        if foreign_currency_id == 2:
                            ...
                            record.retention_amount = (
                                record.facture_amount
                                * (record.related_percentage_tax_base / 100)
                                * (record.related_percentage_tariffs / 100)
                            ) - record.related_amount_sustract_tariffs
                            ...
                            value_rate_inverse = decimal_function.getCurrencyValue(
                                rate=record.invoice_id.foreign_currency_rate,
                                base_currency="VEF",
                                foreign_currency="USD",
                            )
                            record.foreign_retention_amount = (
                                (record.facture_amount * value_rate)
                                * (record.related_percentage_tax_base / 100)
                                * (record.related_percentage_tariffs / 100)
                            ) - record.related_amount_sustract_tariffs
                            record.retention_amount = (
                                record.foreign_retention_amount * value_rate_inverse
                            )
    ...
    def _onchange_foreign_facture_amount(self):
        ...
        for record in self:
            ...
            if record.retention_id and record.retention_id.type in ["out_invoice", "in_invoice"]:
    ...
    def _onchange_foreigh_retention_amount(self):
        ...
        for record in self:
            if (
                record.foreign_retention_amount > record.foreign_iva_amount
                and record.retention_id.type_retention in ["iva"]
            ):
                raise ValidationError(
                    _(
                        "El monto a retener no debe superar al IVA de la factura, por favor verificar."
                    )
                )
            ...
            if not self.env.context.get("noonchange"):
                if (
                    record.retention_id
                    and record.retention_id.type in ["out_invoice"]
                    and record.foreign_currency_rate > 0
                ):
                    if foreign_currency_id == 3:
                        if (
                            record.retention_amount > record.iva_amount
                            ...
                            return {
                                "warning": {
                                    "title": "El monto retenido excedente",
                                    "message": "El monto a retener no debe superar al IVA de la factura, por favor verificar",
                                },
                                "value": {"retention_amount": 0},
                            }
                        ...
                        value_rate = decimal_function.getCurrencyValue(
                            rate=record.foreign_currency_rate,
                            base_currency="VEF",
                            foreign_currency="USD",
                        )
                        _logger.warning(value_rate)
    ...
    retention_id = fields.Many2one(
        "account.retention", "Comprobante", ondelete="cascade", select=True, help="Comprobante"
    )
    ...
    reference_invoice_number = fields.Char(
        string="Número de factura", related="invoice_id.name", store=True
    )
    tax_line = fields.Float(string="Alicuota")
    amount_tax_ret = fields.Float(
        string="Impuesto retenido", help="Total impuesto retenido de la factura"
    )
    base_ret = fields.Float(string="Base imponible", help="Base retenida de la factura")
    imp_ret = fields.Float(string="Impuesto Causado")
    retention_rate = fields.Float(
        compute=_retention_rate,
        store=True,
        string="Portancentaje de Retención",
        help="Porcentaje de Retencion ha aplicar a la factura",
    )
    move_id = fields.Many2one(
        "account.move", "Movimiento Contable", help="Asiento Contable", ondelete="cascade"
    )
    ...
    display_invoice_number = fields.Char(
        string="Display", compute="_compute_fields_combination_iva", store=True
    )
    ...
    payment_concept_id = fields.Many2one(
        "payment.concept", "Concepto de pago", ondelete="cascade", select=True
    )
    ...
    related_percentage_tax_base = fields.Float(
        string="% Base Imponible", compute=_get_value_related, store=True
    )
    related_percentage_tariffs = fields.Float(
        string="% Tarifa", compute=_get_value_related, store=True
    )
    related_amount_sustract_tariffs = fields.Float(
        string="Sustraendo", compute=_get_value_related, store=True
    )
