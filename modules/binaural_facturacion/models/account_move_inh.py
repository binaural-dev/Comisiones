...
class AccountMoveBinauralFacturacion(models.Model):
    ...
    @api.depends('invoice_line_ids.price_total', 'foreign_currency_rate', 'foreign_amount_by_group_base')
    ...
    def create(self, vals_list):
        ...
        if flag:
            for record in res:
                old_rate = record.default_currency_rate()
                # El usuario xxxx ha usado una tasa personalizada, la tasa del sistema para la fecha
                # del pago xxx es de xxxx y ha usada la tasa personalizada xxx.
                display_msg = "El usuario " + self.env.user.name + \
                    " ha usado una tasa personalizada,"
                display_msg += " la tasa del sistema para la fecha " + \
                    str(fields.Date.context_today(record)) + " es de "
                display_msg += str(old_rate) + \
                    " y ha usada la tasa personalizada " + str(round(record.foreign_currency_rate, 2))
    ...
    def _compute_last_payment_date(self):
        for record in self:
            ...
            record.last_payment_date = (
                settlement_date if settlement_date else fields.Datetime.now()
            )
    ...
    def _compute_invoice_taxes_by_group(self):
        ...
        for move in self:
            ...
            move.amount_by_group_base = [
                (
                    group[0].replace("IVA", "Total G").replace("TAX", "Total G"),
                    group[2],
                    group[1],
                    group[4],
                    group[3],
            ...
            foreign_res = {}
            for line in move.invoice_line_ids.filtered(lambda l: any(l.tax_ids)):
                foreign_res.setdefault(
                    line.tax_ids[0].tax_group_id,
                    {'foreign_base':0.0, 'foreign_amount': 0.0}
                )
                amount = line.foreign_subtotal
                foreign_res[line.tax_ids[0].tax_group_id]['foreign_amount'] += (
                    amount * line.tax_ids[0].amount / 100
                )
                foreign_res[line.tax_ids[0].tax_group_id]['foreign_base'] += amount
            foreign_res = sorted(foreign_res.items(), key=lambda l: l[0].sequence)

            move.foreign_amount_by_group = [
                (
                    group.name,
                    amounts['foreign_amount'],
                    amounts['foreign_base'],
                    formatLang(lang_env, amounts['foreign_amount'],
                               currency_obj=move.foreign_currency_id),
                    formatLang(lang_env, amounts['foreign_base'],
                               currency_obj=move.foreign_currency_id),
                    len(foreign_res),
                    group.id
                )
                for group, amounts in foreign_res
            ]
    ...
    def sequence(self):
        ...
        sequence = (
            self.journal_id.series_correlative_sequence_id if series_invoicing
            else self.env["ir.sequence"].search([("code", "=", "invoice.correlative")])
        )
...
class AccountMoveLineBinauralFact(models.Model):
    ...
    @api.depends('move_id.foreign_currency_rate', 'discount','price_unit', 'quantity', 'tax_ids')
    def _amount_all_foreign(self):
        """
        Compute the foreign total amounts of the SO.
        """
        ...
        for order in self:
            ...
            if name_foreign_currency:
                ...
                order.foreign_price_unit = order.price_unit * value_rate
                discount_foreign = ((order.discount * 1/100) * order.foreign_price_unit) * order.quantity
                order.foreign_subtotal = order.foreign_price_unit * order.quantity - discount_foreign
            

    foreign_price_unit = fields.Monetary(string='Precio Alterno', store=True, readonly=True,
                                         compute='_amount_all_foreign', tracking=4, currency_field='foreign_currency_id')
    ...
    foreign_currency_rate = fields.Float(string="Tasa", store=True,
