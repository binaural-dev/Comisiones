    ...
    if self.type in ["out_invoice"]:
        for facture_line_retention in self.env["account.move"].search(
            [
                ...
                ("move_type", "in", ["out_invoice", "out_debit", "out_refund"]),
                ("state", "=", "posted"),
                ("journal_id.fiscal", "=", True),
            ]
        ):
            value_rate = decimal_function.getCurrencyValue(
                rate=facture_line_retention.foreign_currency_rate,
                base_currency=base_currency,
                foreign_currency=foreign_currency,
            )
            if self.type_retention in ["iva"]:
                if (
                    not facture_line_retention.apply_retention_iva
                    and facture_line_retention.amount_tax > 0
                    ...
                    for tax in facture_line_retention.amount_by_group:
                        ...
                        tax_id = self.env["account.tax"].search(
                            [("tax_group_id", "=", tax[6]), ("type_tax_use", "=", "sale")]
                        )
                        if tax_id.amount > 0:
                            data.append(
                                (
                ...
                if (
                    not facture_line_retention.apply_retention_islr
                    and facture_line_retention.payment_state in ["not_paid", "partial"]
                ):
                    data.append(
                        (
                            ...
                            0,
                            {
                                ...
                                "invoice_type": facture_line_retention.move_type,
                                "foreign_facture_amount": facture_line_retention.amount_untaxed
                                * facture_line_retention.foreign_currency_rate,
                                "foreign_facture_total": facture_line_retention.amount_total
                                * facture_line_retention.foreign_currency_rate,
                                "foreign_iva_amount": facture_line_retention.amount_tax
                                * facture_line_retention.foreign_currency_rate,
                                "foreign_currency_rate": facture_line_retention.foreign_currency_rate,
                            },
                        )
...
def search_account(self, ret_line):
    ...
    if self.type in ["out_invoice"]:
        ...
        if not cxc:
            raise UserError(
                "Disculpe, la factura %s no tiene ninguna cuenta por cobrar "
            ) % ret_line.invoice_id.name
    else:
        cxp = False
        for cta in ret_line.invoice_id.line_ids:
        ...
        if not cxp:
            raise UserError(
                "Disculpe, la factura %s no tiene ninguna cuenta por pagar "
            ) % ret_line.invoice_id.name
...
def create_move_invoice_retention(
    ...
    if self.type in ["out_invoice"]:
        ...
        name = _("CXC-RC-" + self.number + "-" + ret_line.invoice_id.name + tax_name)
        line_ret.append(
            (
                0,
                0,
                {
                    ...
                    if amount_edit
                    else self.round_half_up(ret_line.retention_amount, decimal_places),
                },
            )
        )
        line_ret.append(
            (
                ...
                0,
                {
                    ...
                    "account_id": self.partner_id.iva_retention.id
                    if self.type_retention in ["iva"]
                    else self.partner_id.islr_retention.id,
                    "partner_id": self.partner_id.id,
            ...
            move_obj = self.env["account.move"].create(
                {
                    ...
                    if self.type_retention in ["iva"]
                    else "RIS-" + self.number + "-" + ret_line.invoice_id.name,
        ...
        tax_name = " - IVA(" + str(ret_line.tax_line) + "%)"
        name = _("CXP-RC-" + self.number + "-" + ret_line.invoice_id.name + tax_name)
        line_ret.append(
            (
                0,
                0,
...
def create_move_refund_retention(
    self, line_ret, ret_line, account, journal, amount_edit, decimal_places, new_move, move_id
):
    if self.type in ["out_invoice"]:
        tax_name = " - IVA(" + str(ret_line.tax_line) + "%)"
