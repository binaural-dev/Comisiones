...
class BookSaleReport(models.TransientModel):
    ...
    def _sale_book_invoice(self, current_company=False):
        ...
        docs_ids = self.env["account.move"].search(search_domain, order="name asc").ids
        dic = OrderedDict(
            [
                ("Nª de Ope", 0),
                ("Fecha", ""),
        ...
        search_domain_only_retention = [
            ...
            (
                "retention_id.type",
                "in",
                ["out_invoice", "out_refund", "out_debit", "out_contingence"],
            ),
            ("retention_id.type_retention", "=", "iva"),
        ...
        for x in retention_lines_only:
            if x.invoice_id.id not in invoices:
                ...
                if x.invoice_id.move_type in ["out_invoice"] and x.invoice_id.state in ["cancel"]:
                    dict["Tipo Transacción"] = "03-ANU"
                if x.invoice_id.move_type in ["out_refund", "out_debit"] and x.invoice_id.state in [
                    "posted"
                ]:
                    dict["Tipo Transacción"] = "02-REG"
                if x.invoice_id.move_type in ["out_refund", "out_debit"] and x.invoice_id.state in [
                    ...
                    dict["Tipo Transacción"] = "03-ANU"
                dict["Nª de Doc. Afectado"] = (
                    x.invoice_id.reversed_entry_id.name if x.invoice_id.reversed_entry_id else ""
                )
                dict["Total Ventas incluye IVA"] = 0.00
                dict["Total Ventas Exentas"] = 0.00
                dict["Imponible16"] = 0.00
    ...
    def _table_sale_book(self, wizard=False, current_company = False):
        if wizard:
            ...
            wiz = self
        tabla1 = wiz._sale_book_invoice(current_company)
