...
class CalculateCurrency(models.Model):
    ...
    config_main_currency_name = fields.Char(related="company_id.currency_id.name")
    ...
    def _onchange_rate(self):
        ...
        if not float_is_zero(self.vef_rate, 10):
            self.rate = decimal_function.getCurrencyValue(
                rate=self.vef_rate, base_currency="VEF", foreign_currency="USD"
            )
